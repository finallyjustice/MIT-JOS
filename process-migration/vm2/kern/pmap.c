...
i386_vm_init(void)
{
	...
	size_t nenvs;
	nenvs=ROUNDUP(NENV*sizeof(struct Env), PGSIZE);
	envs=(struct Env *)boot_alloc(nenvs, PGSIZE);
	...
	// Your code goes here:
	boot_map_segment(pgdir, (uintptr_t)pages, n, PADDR(pages), PTE_W|PTE_P);
	...
	// LAB 3: Your code here.
	boot_map_segment(pgdir, UENVS, nenvs, PADDR(envs), PTE_U|PTE_P);
	boot_map_segment(pgdir, (uintptr_t)envs, nenvs, PADDR(envs), PTE_W|PTE_P);
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here: 
	boot_map_segment(pgdir, KERNBASE, ROUNDUP(~0x0-KERNBASE, PGSIZE), 0, PTE_W|PTE_P) ;
...
page_init(void)
{
	...
	for(count=1; count<npage; count++)
	{
		pages[count].pp_ref=0;
	}

	for(count=1; count<npage; count++) 
	{
		pa=page2pa(&pages[count]);
		
		if(((pa>=IOPHYSMEM) && (pa<EXTPHYSMEM)) || ((pa>=EXTPHYSMEM) && (pa<PADDR(boot_freemem)))) 
		{
			pages[count].pp_ref=1;

			continue;
		}
					
		LIST_INSERT_HEAD(&page_free_list, &pages[count], pp_link);
	}
}
...
page_alloc(struct Page **pp_store)
{
	...
	temp=LIST_FIRST(&page_free_list);	
	if(temp) 
	{
...
page_free(struct Page *pp)
{
	...
	{
		panic("Cannot free this page! This function should only be called when pp->pp_ref reaches 0!\n");
	}
}
...
boot_map_segment(pde_t *pgdir, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	for(count=0; count<size; count=count+PGSIZE) 
	{
		pte=pgdir_walk(pgdir, (void *)(la+count), 1);
		*pte=pa+count;
		*pte=(*pte)|perm|PTE_P;

		pgdir[PDX(la+count)]=pgdir[PDX(la+count)]|perm;
	}
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	start_va=(uintptr_t)va;
	end_va=(uintptr_t)va+len-1;
...
void showmappings(uint32_t start_va, uint32_t end_va)
{
	...
	{
		pte=pgdir_walk(boot_pgdir, (void *)start_va, 0);

		if((pte!=NULL) && (*pte&PTE_P))
		{	
			cprintf("VirtAddr: 0x%08x - PhysAddr: 0x%08x PermBit: ", start_va, PTE_ADDR(*pte));
		...
		{
			cprintf("VirtAddr: 0x%08x - No Mapping Exists .\n", start_va);
		}
...
void setmappingperm(uint32_t va, uint32_t perm)
{
	...
	pte=pgdir_walk(boot_pgdir, (void *)va, 0);

	if((pte!=NULL) && (*pte&PTE_P))
	{
		//I use 'and' instead of 'or' operation
		*pte=(*pte) & ~0x6;
		*pte=(*pte) | perm;
	}
	else
...
//be careful, programs only works with va
void dumpmeminfo(uint32_t start_va, uint32_t end_va)
{
	...
	while(start_va < end_va) 
	{
		...
		{	
			cprintf("0x%08x ", *((uint32_t*)start_va));
			start_va=start_va+4;
		}
