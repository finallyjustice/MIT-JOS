...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	int result;
	int num_req;
	struct OpenFile *of_instance;
	
	if((result=openfile_lookup(envid, req->req_fileid, &of_instance)) < 0)
	{
		return result;
	}
	...
	if((result=file_read(of_instance->o_file, ret->ret_buf, num_req, of_instance->o_fd->fd_offset)) < 0)
	{
		return result;
	}
	
	of_instance->o_fd->fd_offset=of_instance->o_fd->fd_offset+result;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if((result=file_write(of_instance->o_file, req->req_buf, num_req, of_instance->o_fd->fd_offset)) < 0)
	{
		return result;
	}
...
serve(void)
{
	...
	while (1) {
		...
		ipc_send(whom, r, pg, perm | PTE_SHARE);
